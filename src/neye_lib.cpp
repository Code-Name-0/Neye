...
void feature_extraction_thread(int group_id, State &state)
{
    ...
    try
    {
        ...
        // Open file once to write header if needed
        {
            ...
            {
                header_check
                    << "Header_Length,Protocol_Type,Duration,IAT,IAT_Std,Rate,Srate,"
                       "syn_flag_number,ack_flag_number,fin_flag_number,rst_flag_number,"
                       "psh_flag_number,ece_flag_number,cwr_flag_number,"
                       "TCP,UDP,ICMP,HTTP,HTTPS,DNS,MQTT,SSH,Telnet,SMTP,ARP,DHCP,"
                       "Tot_sum,Min,Max,AVG,Std,Variance,Magnitude,Radius,Covariance,Weight\n";
            }
            header_check.close();
        }
        ...
        while (running)
        {
            ...
            if (current_queue.try_pop(flow_packets, 100))
            {
                if (flow_packets.size() > 1)
                {
                    ...
                    uint32_t num_packets = flow_packets.size();

                    // Initialize feature calculation variables
                    double total_header_length = 0.0;
                    uint8_t protocol_type = 0;
                    double duration_seconds = 0.0;
                    std::vector<double> iat_values;
                    std::vector<uint32_t> packet_sizes;

                    // TCP flag counters
                    uint32_t syn_count = 0, ack_count = 0, fin_count = 0, rst_count = 0;
                    uint32_t psh_count = 0, ece_count = 0, cwr_count = 0;

                    // Protocol counters
                    uint32_t tcp_count = 0, udp_count = 0, icmp_count = 0;
                    uint32_t http_count = 0, https_count = 0, dns_count = 0, mqtt_count = 0;
                    uint32_t ssh_count = 0, telnet_count = 0, smtp_count = 0, arp_count = 0, dhcp_count = 0;

                    double tot_sum = 0.0;
                    int64_t previous_timestamp = -1;

                    // Process each packet in the flow
                    for (size_t i = 0; i < flow_packets.size(); i++)
                    {
                        const auto &pkt = flow_packets[i];

                        // Validate packet size
                        if (pkt.data.size() < 14)
                            continue; // Minimum Ethernet header size

                        uint32_t packet_size = pkt.data.size();
                        ...
                        {
                            double current_iat = (pkt.timestamp - previous_timestamp) / 1e6; // Convert to seconds
                            iat_values.push_back(current_iat);
                        }
                        ...
                        if (ethernet_type == 0x0800 && pkt.data.size() >= ip_begin + 20)
                        {
                            uint8_t ip_header_length = (pkt.data[ip_begin] & 0x0F) * 4;
                            total_header_length += ip_header_length;

                            // Validate IP header length
                            if (ip_header_length < 20 || pkt.data.size() < ip_begin + ip_header_length)
                                continue;

                            uint8_t protocol = pkt.data[protocol_offset];
                            if (protocol_type == 0)
                                protocol_type = protocol; // Set protocol for flow
                            ...
                            {
                                tcp_count++;
                                size_t tcp_begin = ip_begin + ip_header_length;
                                ...
                                if (pkt.data.size() > tcp_begin + 13)
                                {
                                    ...
                                    // Extract ports for protocol detection
                                    if (pkt.data.size() > tcp_begin + 3)
                                    {
                                        ...
                                        if (src_port == 80 || dst_port == 80)
                                            ...
                                            dns_count++;
                                        else if (src_port == 1883 || dst_port == 1883 || src_port == 8883 || dst_port == 8883)
                                            mqtt_count++;
                            ...
                            {
                                udp_count++;
                                size_t udp_begin = ip_begin + ip_header_length;

                                if (pkt.data.size() > udp_begin + 3)
                                {
                                    uint16_t src_port = (pkt.data[udp_begin] << 8) | pkt.data[udp_begin + 1];
                                    uint16_t dst_port = (pkt.data[udp_begin + 2] << 8) | pkt.data[udp_begin + 3];
                    ...
                    if (flow_packets.size() > 1)
                    {
                        duration_seconds = (flow_packets.back().timestamp - flow_packets.front().timestamp) / 1e6;
                    }
                    duration_seconds = duration_seconds > 0 ? duration_seconds : epsilon;

                    // Calculate statistical features
                    double min_size = packet_sizes.empty() ? 0.0 : *std::min_element(packet_sizes.begin(), packet_sizes.end());
                    double max_size = packet_sizes.empty() ? 0.0 : *std::max_element(packet_sizes.begin(), packet_sizes.end());
                    double avg_size = packet_sizes.empty() ? 0.0 : tot_sum / packet_sizes.size();

                    // Calculate standard deviation and variance
                    double variance = 0.0, std_dev = 0.0;
                    if (packet_sizes.size() > 1)
                    {
                        double sum_squares = 0.0;
                        for (uint32_t size : packet_sizes)
                        {
                            sum_squares += (size - avg_size) * (size - avg_size);
                        }
                        variance = sum_squares / (packet_sizes.size() - 1);
                        std_dev = std::sqrt(variance);
                    }

                    // Calculate IAT statistics
                    double avg_iat = 0.0, iat_std = 0.0;
                    if (!iat_values.empty())
                    {
                        double iat_sum = std::accumulate(iat_values.begin(), iat_values.end(), 0.0);
                        avg_iat = iat_sum / iat_values.size();

                        if (iat_values.size() > 1)
                        {
                            double iat_var_sum = 0.0;
                            for (double iat : iat_values)
                            {
                                iat_var_sum += (iat - avg_iat) * (iat - avg_iat);
                            }
                            iat_std = std::sqrt(iat_var_sum / (iat_values.size() - 1));
                        }
                    }

                    // Calculate rates
                    double rate = tot_sum / duration_seconds;      // bytes per second
                    double srate = num_packets / duration_seconds; // packets per second

                    // Calculate geometric features
                    double magnitude = std::sqrt(avg_size * avg_size + avg_iat * avg_iat + rate * rate);
                    double radius = magnitude; // Distance from origin

                    // Calculate covariance between packet size and IAT
                    double covariance = 0.0;
                    if (packet_sizes.size() > 1 && iat_values.size() > 0)
                    {
                        size_t min_samples = std::min(packet_sizes.size() - 1, iat_values.size());
                        if (min_samples > 0)
                        {
                            double sum_prod = 0.0;
                            for (size_t i = 0; i < min_samples; i++)
                            {
                                sum_prod += (packet_sizes[i + 1] - avg_size) * (iat_values[i] - avg_iat);
                            }
                            covariance = sum_prod / min_samples;
                        }
                    }

                    // Calculate weight (simple heuristic based on flow importance)
                    double weight = (num_packets * avg_size) / (duration_seconds + 1.0);

                    // Assign features to struct
                    flow_features.Header_Length = total_header_length;
                    flow_features.Protocol_Type = protocol_type;
                    flow_features.Duration = duration_seconds;
                    flow_features.IAT = avg_iat;
                    flow_features.IAT_Std = iat_std; // Now stored in struct
                    flow_features.Rate = rate;
                    flow_features.Srate = srate;

                    // TCP flags as ratios
                    double total_packets_d = static_cast<double>(num_packets);
                    flow_features.syn_flag_number = total_packets_d > 0 ? syn_count / total_packets_d : 0.0;
                    flow_features.ack_flag_number = total_packets_d > 0 ? ack_count / total_packets_d : 0.0;
                    flow_features.fin_flag_number = total_packets_d > 0 ? fin_count / total_packets_d : 0.0;
                    flow_features.rst_flag_number = total_packets_d > 0 ? rst_count / total_packets_d : 0.0;
                    flow_features.psh_flag_number = total_packets_d > 0 ? psh_count / total_packets_d : 0.0;
                    flow_features.ece_flag_number = total_packets_d > 0 ? ece_count / total_packets_d : 0.0;
                    flow_features.cwr_flag_number = total_packets_d > 0 ? cwr_count / total_packets_d : 0.0;

                    // Protocol indicators (binary)
                    flow_features.TCP = tcp_count > 0 ? 1.0 : 0.0;
                    flow_features.UDP = udp_count > 0 ? 1.0 : 0.0;
                    flow_features.ICMP = icmp_count > 0 ? 1.0 : 0.0;
                    flow_features.HTTP = http_count > 0 ? 1.0 : 0.0;
                    flow_features.HTTPS = https_count > 0 ? 1.0 : 0.0;
                    flow_features.DNS = dns_count > 0 ? 1.0 : 0.0;
                    flow_features.MQTT = mqtt_count > 0 ? 1.0 : 0.0;     // Now in struct
                    flow_features.SSH = ssh_count > 0 ? 1.0 : 0.0;       // Now in struct
                    flow_features.Telnet = telnet_count > 0 ? 1.0 : 0.0; // Now in struct
                    flow_features.SMTP = smtp_count > 0 ? 1.0 : 0.0;
                    flow_features.ARP = arp_count > 0 ? 1.0 : 0.0;
                    flow_features.DHCP = dhcp_count > 0 ? 1.0 : 0.0;

                    // Statistical features
                    flow_features.Tot_sum = tot_sum;
                    flow_features.Min = min_size;
                    flow_features.Max = max_size;
                    flow_features.AVG = avg_size;
                    flow_features.Std = std_dev;
                    flow_features.Variance = variance;
                    flow_features.Magnitue = magnitude; // Note: struct has typo "Magnitue"
                    flow_features.Radius = radius;
                    flow_features.Covariance = covariance;
                    flow_features.Weight = weight;
                    ...
                    // Output features in exact order as specified
                    csv_file
                        << flow_features.Header_Length << ','
                        << flow_features.Protocol_Type << ','
                        << flow_features.Duration << ','
                        << flow_features.IAT << ','
                        << flow_features.IAT_Std << ',' // Now from struct
                        << flow_features.Rate << ','
                        << flow_features.Srate << ','
                        ...
                        << flow_features.HTTPS << ','
                        << flow_features.DNS << ','
                        << flow_features.MQTT << ','   // Now from struct
                        << flow_features.SSH << ','    // Now from struct
                        << flow_features.Telnet << ',' // Now from struct
                        << flow_features.SMTP << ','
                        << flow_features.ARP << ','
                        << flow_features.DHCP << ','
                        << flow_features.Tot_sum << ','
                        << flow_features.Min << ','
                        << flow_features.Max << ','
                        << flow_features.AVG << ','
                        << flow_features.Std << ','
                        << flow_features.Variance << ','
                        << flow_features.Magnitue << ',' // Note: keeping typo from struct
                        << flow_features.Radius << ','
                        << flow_features.Covariance << ','
                        << flow_features.Weight
                        << '\n';
                    ...
                    std::cout << flow_features.Header_Length << ','
                              << flow_features.Protocol_Type << ','
                              << flow_features.Duration << ','
                              << flow_features.IAT << ','
                              << flow_features.IAT_Std << ',' // Now from struct
                              ...
                              << flow_features.Std << ','
                              << flow_features.Variance << ','
                              << flow_features.Magnitue << ','
                              << flow_features.Radius << ','
                              << flow_features.Covariance << ','
                              << flow_features.Weight << std::endl;

                    LOG_THREAD_INFO("FeatureExtract-" + std::to_string(group_id), "Features",
                                    "Processed flow with " + std::to_string(num_packets) +
        ...
        while (running)
        {
            ...
            for (int group_id = 0; group_id < number_of_groups; group_id++)
            {
                if (features_queues[group_id].try_pop(features, 0))
                {
                    ...
                    // Create JSON payload with all 36 CIC IoMT 2024 features in correct order
                    json json_payload = {
                        {"device", device_ip},
                        {"device_name", device_name},
                        {"device_mac", device_mac},
                        {"group_id", group_id},
                        {"features", {{"Header_Length", features.Header_Length}, {"Protocol_Type", features.Protocol_Type}, {"Duration", features.Duration}, {"IAT", features.IAT}, {"IAT_Std", features.IAT_Std}, {"Rate", features.Rate}, {"Srate", features.Srate}, {"syn_flag_number", features.syn_flag_number}, {"ack_flag_number", features.ack_flag_number}, {"fin_flag_number", features.fin_flag_number}, {"rst_flag_number", features.rst_flag_number}, {"psh_flag_number", features.psh_flag_number}, {"ece_flag_number", features.ece_flag_number}, {"cwr_flag_number", features.cwr_flag_number}, {"TCP", features.TCP}, {"UDP", features.UDP}, {"ICMP", features.ICMP}, {"HTTP", features.HTTP}, {"HTTPS", features.HTTPS}, {"DNS", features.DNS}, {"MQTT", features.MQTT}, {"SSH", features.SSH}, {"Telnet", features.Telnet}, {"SMTP", features.SMTP}, {"ARP", features.ARP}, {"DHCP", features.DHCP}, {"Tot_sum", features.Tot_sum}, {"Min", features.Min}, {"Max", features.Max}, {"AVG", features.AVG}, {"Std", features.Std}, {"Variance", features.Variance}, {"Magnitude", features.Magnitue}, {"Radius", features.Radius}, {"Covariance", features.Covariance}, {"Weight", features.Weight}}}};
...
void logger_thread(State &state)
{
    ...
    try
    {
        ...
        { // Generate the current snapshot of system state
            ...
            std::this_thread::sleep_for(std::chrono::milliseconds(1000)); // two seconds
        }
    }
    catch (const std::exception &e)
    {
...
{
    const char *url = std::getenv("STATS_API_URL");
    if (!url)
    {
        LOG_ERROR("Dashboard", "STATS_API_URL environment variable not set");
        return;
    }
